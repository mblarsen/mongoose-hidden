...
function copyRealValues(transformation, doc) {
  return pathname => {
    const { finalTransform, schema, transformed } = transformation
    const schemaType = schema.path(pathname)
    if (shouldHide(transformation, schemaType, doc, pathname)) {
 ...
 * @returns {Boolen} true of pathname should be hidden
 */
function shouldHide({ options, target, transformed }, schema, doc, pathname) {
